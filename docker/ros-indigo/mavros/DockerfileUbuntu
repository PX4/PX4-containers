...
RUN apt-get update \
    ...
    && apt-get -y --no-install-recommends install wget sudo openssl \
    && apt-get clean \
    && rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/*

# Main ROS Setup
COPY scripts/install-ros-base-ubuntu.bash /scripts/install-ros-base-ubuntu.bash
RUN chmod +x -R /scripts/* && chown -R root:root /scripts/*
RUN bash /scripts/install-ros-base-ubuntu.bash
